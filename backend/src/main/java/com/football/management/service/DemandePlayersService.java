...
public class DemandePlayersService {
        ...
        try {
                ...
                WHERE i.ct_intervenant_id IN (
                    ...
                    INNER JOIN (
                        ...
                        WHERE (teamPlayer.ct_season_id < ?
                               ...
                               AND teamPlayer.ct_type_licence_id NOT IN (5, 6))
                           OR (teamPlayer.ct_season_id = ?
                               AND teamPlayer.ct_intervenant_type_id = 1
                               AND teamPlayer.ct_type_licence_id IN (4, 11))
            ...
            return results.stream()
                .map(row -> {
                    ...
                    joueur.put("teamId", teamId);
                    joueur.put("seasonId", currentSeasonId);
                    joueur.put("regimeId", regimeId != null ? regimeId : 1);
                    return joueur;
                })
                .collect(Collectors.toList());
    ...
    /**
     * Récupère les joueurs éligibles pour RETOUR PRÊT (Type 3)
     *
     * LOGIQUE:
     * - Joueurs qui étaient en PRÊT (Type 5) ou RENOUVELLEMENT SPÉCIAL (Type 6)
     * - Dans votre club (teamId)
     * - Dans les 4 dernières saisons (seasonId - 4 à seasonId)
     * - Saison la plus récente pour chaque joueur (MAX)
     */
    public List<Map<String, Object>> getJoueursEligiblesRetourPret(Long teamId, Long regimeId, Long currentSeasonId) {
        try {
            String sql = """
                SELECT
                    i.ct_intervenant_id as id,
                ...
                WHERE i.ct_intervenant_id IN (
                    ...
                    WHERE beanDiv.ct_team_id = ?
                      AND beanDiv.ct_type_licence_id IN (5, 6)
            ...
            System.err.println("ERREUR getJoueursEligiblesRetourPret: " + e.getMessage());
            e.printStackTrace();
            return new ArrayList<>();
        }
    }
    ...
    /**
     * Récupère les joueurs éligibles pour RETOUR MUTATION (Type 9)
     *
     * RÈGLE MÉTIER:
     * - Le joueur doit avoir fait une MUTATION (Type 4) vers une autre équipe dans la saison en cours
     * - Son équipe d'ORIGINE (avant la mutation) doit être l'équipe demandée (teamId)
     * - Cela permet à l'équipe d'origine de récupérer son joueur après une mutation
     *
     * LOGIQUE SQL:
     * 1. Trouve les joueurs avec Type 4 (MUTATION) dans la saison en cours
     * 2. Pour chaque joueur, trouve son enregistrement PRÉCÉDENT (avant la mutation)
     * 3. Vérifie que l'équipe de cet enregistrement précédent = teamId
     */
    public List<Map<String, Object>> getJoueursEligiblesRetourMutation(Long teamId, Long regimeId, Long currentSeasonId) {
        try {
            String sql = """
                SELECT
                    i.ct_intervenant_id as id,
            ...
            System.err.println("ERREUR getJoueursEligiblesRetourMutation: " + e.getMessage());
            e.printStackTrace();
            return new ArrayList<>();
        }
    }
    ...
    private String getTypeLicenceLabel(BigDecimal typeLicenceId) {
        ...
        switch (id) {
            ...
            case 3: return "Retour Prêt";
            case 4: return "Mutation";
            case 5: return "Prêt";
            case 6: return "Renouvellement Spécial";
            case 8: return "Transfert";
            case 9: return "Retour Mutation";
            case 10: return "Surclassement";
            case 11: return "Libre (Amateur)";
            case 12: return "Transfert Libre";
            case 14: return "Transfert à l'étranger";
            case 15: return "Ancien Licencié";
