...
public class DemandePlayersController {
	...
	public ResponseEntity<?> createNouveauJoueur(@RequestBody DemandePlayersDTO demandeDTO,
			@Parameter(description = "ID de l'utilisateur", example = "1") @RequestParam(required = false, defaultValue = "1") Long userId) {
		try {
			// S'assurer que c'est bien une NOUVELLE LICENCE (Type 1)
			demandeDTO.setTypeLicenceId(BigDecimal.valueOf(1));

			// ✅ VALIDATION COMPLÈTE via ValidationOrchestrator
			ValidationResult validationResult = validationOrchestrator.validateDemandeComplete(demandeDTO);
	...
	public ResponseEntity<?> createPret(@PathVariable BigDecimal joueurId, @RequestBody DemandePlayersDTO demandeDTO,
		...
		try {
			...
			// ✅ VALIDATION COMPLÈTE avec ValidationOrchestrator
			ValidationResult validationResult = validationOrchestrator.validateDemandeComplete(demandeDTO);
	...
	@GetMapping("/joueurs-eligibles-retour-pret")
	public ResponseEntity<List<Map<String, Object>>> getJoueursEligiblesRetourPret(@RequestParam Long teamId,
			@RequestParam Long regimeId, @RequestParam Long currentSeasonId) {
		try {
			List<Map<String, Object>> joueurs = demandePlayersService.getJoueursEligiblesRetourPret(teamId,
					regimeId, currentSeasonId);

			return ResponseEntity.ok(joueurs);

		} catch (Exception e) {
			logger.error("Erreur lors de la récupération des joueurs éligibles pour retour prêt", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

	@GetMapping("/joueurs-eligibles-retour-mutation")
	public ResponseEntity<List<Map<String, Object>>> getJoueursEligiblesRetourMutation(@RequestParam Long teamId,
			@RequestParam Long regimeId, @RequestParam Long currentSeasonId) {
		try {
			List<Map<String, Object>> joueurs = demandePlayersService.getJoueursEligiblesRetourMutation(teamId,
					regimeId, currentSeasonId);

			return ResponseEntity.ok(joueurs);

		} catch (Exception e) {
			logger.error("Erreur lors de la récupération des joueurs éligibles pour retour mutation", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}
